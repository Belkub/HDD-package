...
try:
    ...
    with open('D:\\Documents\\ГНБ\\d.pkl', 'rb') as f:
        d = pickle.load(f)
        ddf = pickle.loads(d)
    df = ddf()
    ...
    if n3:
        ww = 1
        w = {'бесполимерный базовый бентонит Альбрехта-CV':1, 'низковязкий полимерный бентонит Альбрехта-LV':2, 'средневязкий полимерный бентонит Альбрехта-MV':3, 'высоковязкий полимерный бентонит Альбрехта-HV':4}
